...
TEMPLATE_MODEL_NAME = '<TEST_MODEL_NAME>'
...
def replace_template_string(line):
    ...
    if TestModelName_str:
        line = line.replace(TEMPLATE_MODEL_NAME, TestModelName_str)
    if TestCaseName_str:
    ...
    for line in body_buf:
        ...
        if TEMPLATE_TEST_ITEM_BODY in line:
            for inpline in input_buf:
                ...
                for ll in buf:
                    InputParams_str = 'u8"'+'",u8"'.join(params)+'"'
