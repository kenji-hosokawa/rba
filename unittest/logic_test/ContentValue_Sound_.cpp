...
/**
 * ContentValue_Sound_.cpp
 */

#include <vector>
#include <string>
#include "RBAJsonParser.hpp"
#include "ContentValue_Sound_.hpp"
...
ContentValue_Sound_::ContentValue_Sound_(void)
{
}
...
void ContentValue_Sound_::SetUp(void)
{
...
void ContentValue_Sound_::TearDown(void)
{
  delete arb;
  delete model;
  arb = nullptr;
...
TEST_F(ContentValue_Sound_, Test_true) {
  ...
  elems = {u8"Assert",u8"A",u8"優先度（高）ゾーン",u8"コンテンツ優先度（高）"};
  {
    std::string kind = elems[1];
  ...
  elems = {u8"Assert",u8"A",u8"優先度（低）ゾーン",u8"NULL"};
  {
    std::string kind = elems[1];
    std::string target = elems[2];
    std::string expect = elems[3];
...
TEST_F(ContentValue_Sound_, Test_false) {
  std::vector<std::string> elems;
  std::unique_ptr<RBAResult> result;
  ...
  elems = {u8"Assert",u8"A",u8"優先度（低）ゾーン",u8"コンテンツ優先度（低）"};
  {
    std::string kind = elems[1];
    std::string target = elems[2];
    std::string expect = elems[3];
...
TEST_F(ContentValue_Sound_, Test_exception) {
  std::vector<std::string> elems;
  std::unique_ptr<RBAResult> result;
