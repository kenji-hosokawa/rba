...
IsHidden_pre_::IsHidden_pre_(void)
{
}
...
void IsHidden_pre_::TearDown(void)
{
  delete arb;
  delete model;
  arb = nullptr;
...
TEST_F(IsHidden_pre_, Test_true) {
  ...
  elems = {u8"execute"};
  {
    std::string unique_name = u8"AAA";
    result = arb->execute(); // empty
    EXPECT_EQ(rba::RBAResultStatusType::SUCCESS, result->getStatusType());
  }
...
TEST_F(IsHidden_pre_, Test_false) {
  std::vector<std::string> elems;
  std::unique_ptr<RBAResult> result;
