...
/**
 * IsGreaterThanEqualOperator.cpp
 */

#include <vector>
#include <string>
#include "RBAJsonParser.hpp"
#include "IsGreaterThanEqualOperator.hpp"
...
IsGreaterThanEqualOperator::IsGreaterThanEqualOperator(void)
{
}
...
void IsGreaterThanEqualOperator::SetUp(void)
{
...
void IsGreaterThanEqualOperator::TearDown(void)
{
  delete arb;
  delete model;
  arb = nullptr;
...
TEST_F(IsGreaterThanEqualOperator, Test_true) {
  std::vector<std::string> elems;
  std::unique_ptr<RBAResult> result;

  elems = {u8"C",u8"ON",u8"コンテンツ優先度（低）",u8"NORMAL"};
  {
  ...
  elems = {u8"C",u8"ON",u8"コンテンツ優先度（高）",u8"WARNING"};
  {
    std::string kind = elems[1];
    std::string cont = elems[2];
    std::string state = elems[3];
  ...
  elems = {u8"Assert",u8"A",u8"優先度（低）エリア",u8"NULL"};
  {
    std::string kind = elems[1];
  ...
  elems = {u8"Assert",u8"A",u8"優先度（高）エリア",u8"コンテンツ優先度（高）"};
  {
    std::string kind = elems[1];
    std::string target = elems[2];
    std::string expect = elems[3];
  ...
  elems = {u8"C",u8"ON",u8"コンテンツ優先度（低）",u8"WARNING"};
  {
    std::string kind = elems[1];
    std::string cont = elems[2];
    std::string state = elems[3];
  ...
  elems = {u8"C",u8"ON",u8"コンテンツ優先度（高）",u8"NORMAL"};
  {
    std::string kind = elems[1];
    std::string cont = elems[2];
    std::string state = elems[3];
...
TEST_F(IsGreaterThanEqualOperator, Test_false) {
  std::vector<std::string> elems;
  std::unique_ptr<RBAResult> result;
  ...
  elems = {u8"Assert",u8"A",u8"優先度（低）エリア",u8"コンテンツ優先度（低）"};
  {
    std::string kind = elems[1];
    std::string target = elems[2];
    std::string expect = elems[3];
