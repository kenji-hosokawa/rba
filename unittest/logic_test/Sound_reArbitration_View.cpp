...
/**
 * Sound_reArbitration_View.cpp
 */

#include <vector>
#include <string>
#include "RBAJsonParser.hpp"
#include "Sound_reArbitration_View.hpp"
...
Sound_reArbitration_View::Sound_reArbitration_View(void)
{
}
...
void Sound_reArbitration_View::SetUp(void)
{
...
void Sound_reArbitration_View::TearDown(void)
{
  delete arb;
  delete model;
  arb = nullptr;
...
TEST_F(Sound_reArbitration_View, Test_1) {
  ...
  elems = {u8"Assert",u8"A",u8"エリアB",u8"表示コンテンツB2"};
  {
    std::string kind = elems[1];
    std::string target = elems[2];
    std::string expect = elems[3];
  ...
  elems = {u8"Assert",u8"A",u8"ゾーンB",u8"音コンテンツB2"};
  {
    std::string kind = elems[1];
    std::string target = elems[2];
    std::string expect = elems[3];
