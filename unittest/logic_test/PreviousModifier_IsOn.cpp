...
#include "PreviousModifier_IsOn.hpp"

namespace {

using namespace rba;

PreviousModifier_IsOn::PreviousModifier_IsOn() {}
PreviousModifier_IsOn::~PreviousModifier_IsOn() {}

void PreviousModifier_IsOn::SetUp()
{
...
void PreviousModifier_IsOn::TearDown()
{
  delete arb_;
  delete model_;
  arb_ = nullptr;
...
bool PreviousModifier_IsOn::isSuccess(const std::string& msg)
{
  return RBAResultStatusType::SUCCESS == result_->getStatusType();
}

bool PreviousModifier_IsOn::isSatisfiedConstraints(const std::string& msg)
{
  return result_->satisfiesConstraints();
}

bool PreviousModifier_IsOn::isTrue(const std::string& msg, bool res)
{
  return res;
}

std::string PreviousModifier_IsOn::getAllocated(const std::string& msg,
...
bool PreviousModifier_IsOn::isCanceled(const std::string& msg,
...
bool PreviousModifier_IsOn::isActive(const std::string& msg, const std::string& sceneName)
{
...
int32_t PreviousModifier_IsOn::getProperty(const std::string& msg,
...
bool PreviousModifier_IsOn::isEnableAllocatable(const std::string& msg,
                                const std::string& allocName)
{
  const RBAArea* area = model_->findArea(allocName);
  if(area) {
...
bool PreviousModifier_IsOn::isEnableContext(const std::string& msg,
...
bool PreviousModifier_IsOn::isEnableScene(const std::string& msg,
                          const std::string& sceneName)
{
  const RBAScene* scene = model_->findScene(sceneName);
...
bool PreviousModifier_IsOn::isAttenuated(const std::string& msg,
...
// 0001  #start 1
TEST_F(PreviousModifier_IsOn, Test_01)
{
  ...
  // 0018  Assert,A,エリアA,コンテンツA1
  EXPECT_EQ(u8"コンテンツA1",getAllocated("line=18", u8"エリアA"));
  // 0019  Assert,A,エリアB,
  EXPECT_EQ(u8"",getAllocated("line=19", u8"エリアB"));
  // 0020  Assert,S,s1_Local,on
  ...
  // 0024  Assert,S,s1_Local,on
  EXPECT_TRUE(isActive("line=24", u8"s1_Local"));
}

// 0026  #start 2
TEST_F(PreviousModifier_IsOn, Test_02)
{
  std::list<std::string> contexts;
...
// 0046  #start 3
TEST_F(PreviousModifier_IsOn, Test_03)
{
  std::list<std::string> contexts;
  ...
  // 0063  Assert,A,エリアA,コンテンツA2
  EXPECT_EQ(u8"コンテンツA2",getAllocated("line=63", u8"エリアA"));
  // 0064  Assert,A,エリアB,
  EXPECT_EQ(u8"",getAllocated("line=64", u8"エリアB"));
...
// 0071  #start 4
TEST_F(PreviousModifier_IsOn, Test_04)
{
  std::list<std::string> contexts;
